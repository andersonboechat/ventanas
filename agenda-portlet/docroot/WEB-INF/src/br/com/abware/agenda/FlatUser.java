...
public class FlatUser extends UserAdapter {
	...
	public static List<User> getResidents(Organization organization) {
		List<User> users = new ArrayList<User>();

		try {
			Role owner = RoleLocalServiceUtil.getRole(organization.getCompanyId(), RoleConstants.ORGANIZATION_OWNER);
			Role resident = RoleLocalServiceUtil.getRole(organization.getCompanyId(), RoleConstants.ORGANIZATION_ADMINISTRATOR);

			List<User> orgUsers = UserLocalServiceUtil.getOrganizationUsers(organization.getOrganizationId());
			
			for(User user : orgUsers) {
				if (UserGroupRoleLocalServiceUtil.hasUserGroupRole(user.getUserId(), organization.getGroupId(), owner.getRoleId()) ||
						UserGroupRoleLocalServiceUtil.hasUserGroupRole(user.getUserId(), organization.getGroupId(), resident.getRoleId())) {
					users.add(user);
				}
			}
		} catch (PortalException e) {
